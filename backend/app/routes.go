...
func SetupMux() *http.ServeMux {
	...
	// auth handlers
	mux.HandleFunc("/api/v1/auth/status", auth.CheckAuthHandler)
	mux.HandleFunc("/api/v1/auth/refresh", MW.AuthMiddleware(handlers.RefreshHandler))
	...
	// profile handlers
	mux.HandleFunc("/api/v1/profile/{name}", MW.AuthMiddleware(P.ProfileHandler))
	mux.HandleFunc("/api/v1/settings/{type}", MW.AuthMiddleware(P.ProfileSettingsHandler))
	...
	mux.HandleFunc("/api/v1/set/{type}", MW.AuthMiddleware(handlers.SetHandler))
	mux.HandleFunc("/api/v1/upload", MW.AuthMiddleware(handlers.UploadHandler))
