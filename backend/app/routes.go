...
func SetupMux() http.Handler {
	...
	// profile handlers
	mux.HandleFunc("GET /api/v1/profile/{name}", MW.AuthMiddleware(PH.ProfileHandler))
	mux.HandleFunc("POST /api/v1/settings/{type}", MW.AuthMiddleware(PH.ProfileSettingsHandler))
	...
	mux.HandleFunc("POST /api/v1/upload", UH.UploadHandler)
